...
export abstract class BaseRepository<IT, RT> implements RepositoryWriter<IT, RT>, RepositoryReader<RT> {
  ...
  count({ where, include, distinct, col }: { where?: WhereOptions<any>, include?: any, distinct?:boolean, col?:string }): Promise<number> {
    return this.model.count({ where, include, distinct, col });
  }
  ...
  async query({ functionName, params, replacements }: { functionName: string; params?: string, replacements?: object; }): Promise<any> {
