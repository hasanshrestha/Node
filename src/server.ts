...
class Server {
  ...
  public async start() {
    ...
    const server = new ApolloServer<BaseContext>({
      ...
      includeStacktraceInErrorResponses: environment === EnvironmentEnum.production ? false : true,
      cache: 'bounded',
      logger: this.logger,
      formatError: (formattedError: GraphQLFormattedError) => {
        ...
        return {
          ...formattedError,
          message: i18nProvider.__(formattedError.message)
        };
      }
    });
    ...
    await new Promise<void>((resolve) => {
      ...
      console.info(`🚀  Apollo server ready at: `.blue + `http://localhost:${port}/graphql`.blue.bold);
    });
  }
  ...
  private corsOptions: CorsOptions = {
    origin: function (origin, callback) {
      if (!origin || corsWhitelist.indexOf(origin) !== -1) {
        ...
        callback(
          new GraphQLError('Not allowed by CORS', {
            extensions: {
              code: 'BAD_USER_INPUT',
              argumentName: 'cors',
            },
